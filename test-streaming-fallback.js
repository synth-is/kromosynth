#!/usr/bin/env node
/**
 * Test streaming render fallback for genome with unconnected additive node
 */
...
// Genome with additive node but no CPPN connections
const GENOME_ID = '01JF0WF34MKYB3T4N9X7W67EVP';
...
async function testFallback() {
    ...
    console.log('STREAMING FALLBACK TEST');
    console.log('='.repeat(80));
    console.log();

    console.log('📂 Loading genome with unconnected additive node...');
    ...
    console.log('🎵 Rendering with STREAMING mode (should fall back to standard)...');
    ...
    if (nans === 0 && infs === 0) {
        console.log('✅ SUCCESS: Fallback to standard rendering worked!');
        if (valid === 0 && zeros === samples.length) {
            console.log('   (Result is silence - expected for genome with only unconnected nodes)');
        }
        console.log('='.repeat(80));
        process.exit(0);
    } else {
